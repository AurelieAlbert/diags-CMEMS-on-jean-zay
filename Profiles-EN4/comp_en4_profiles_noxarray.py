...
def loc(fileEN4,prof):
    ...
    ds=Dataset(meshfile, 'r')
    lat=ds.variables['nav_lat']
    lon=ds.variables['nav_lon']
    latmin,latmax,lonmin,lonmax=(np.min(lat),np.max(lat),np.min(lon),np.max(lon))
    tmask=ds.variables['tmask'] 
    ...
    lonen4=dsen4.variables['LONGITUDE'][prof]
    
    if (lonmin<lonen4)&(lonen4<lonmax)&(latmin<laten4)&(laten4<latmax):
...
def check_prof_in_ocean(i0,j0):
    ...
    dsN=Dataset(meshfile,'r')
    tmaskN=dsN.variables['tmask']
...
def check_number_profile(fileEN4,prof,i0,j0,dmap=0):
    ...
    lat=ds.variables['nav_lat'][j0-gdpts:j0+gdpts,i0-gdpts:i0+gdpts]
    lon=ds.variables['nav_lon'][j0-gdpts:j0+gdpts,i0-gdpts:i0+gdpts]
    depth=ds.variables['gdept_1d'][0]
    tmask=ds.variables['tmask'][0,:,j0-gdpts:j0+gdpts,i0-gdpts:i0+gdpts]
    # Stack the variables
    xr_lon=xr.DataArray(lon, dims=("y", "x"))   
    lon_stacked = xr_lon.stack(profile=('x', 'y'))
    xr_lat=xr.DataArray(lat, dims=("y", "x"))   
    lat_stacked = xr_lat.stack(profile=('x', 'y'))
    xr_mask=xr.DataArray(tmask, dims=("depth", "y","x"))   
    mask_stacked = xr_mask.stack(profile=('x', 'y'))
    #Get the depth at every grid point
    d,ly,lx=tmask.shape
    depthmod2d=np.zeros([lx,ly])
    for j in np.arange(ly):
        for i in np.arange(lx):
            depthmod2d[j,i]=depth[np.min(np.where(tmask[:,j,i]<1))]
...
def process_one_profile(prof,infos,dirmod,config,case,meshfile,diren4,radius_max,depthmin,period,number_of_model_profiles,debug_plot=False):
        ...
        # Read model files
        gdpts=np.int(np.round(radius_max*60))
        lent=len(list_filesmod_T)*24
        dsT=Dataset(list_filesmod_T[0],'r')
        lonmod=dsT.variables['nav_lon'][j0-gdpts:j0+gdpts,i0-gdpts:i0+gdpts]
        latmod=dsT.variables['nav_lat'][j0-gdpts:j0+gdpts,i0-gdpts:i0+gdpts]
        depthmod=dsT.variables['deptht']
        ttempmod=dsT.variables['votemper'][:,:,j0-gdpts:j0+gdpts,i0-gdpts:i0+gdpts]
        lend,leny,lenx=ttempmod.shape[1],ttempmod.shape[2],ttempmod.shape[3]
        tempmod=np.zeros([lent,lend,leny,lenx])
        saltmod=np.zeros([lent,lend,leny,lenx])
        tt=0
        for fileT in list_filesmod_T:
            dsT=Dataset(fileT,'r')
            ttempmod=dsT.variables['votemper'][:,:,j0-gdpts:j0+gdpts,i0-gdpts:i0+gdpts]
            tempmod[np.int(tt*24):np.int(tt*24+24),:,:,:]=ttempmod
            tt+=1
        tt=0
        for fileS in list_filesmod_S:
            dsS=Dataset(fileS,'r')
            ssaltmod=dsS.variables['vosaline'][:,:,j0-gdpts:j0+gdpts,i0-gdpts:i0+gdpts]
            saltmod[np.int(tt*24):np.int(tt*24+24),:,:,:]=ssaltmod
            tt+=1
        
        ds=Dataset(meshfile,'r')
        maskmod=ds.variables['tmask'][0,:,j0-gdpts:j0+gdpts,i0-gdpts:i0+gdpts]
        maskmod0=ds.variables['tmask'][0,0,j0-gdpts:j0+gdpts,i0-gdpts:i0+gdpts]
        ...
        xr_lonmod=xr.DataArray(lonmod, dims=("y", "x"))   
        lon_stacked = xr_lonmod.stack(profile=('x', 'y'))
        xr_latmod=xr.DataArray(latmod, dims=("y", "x"))   
        lat_stacked = xr_latmod.stack(profile=('x', 'y'))
        xr_maskmod=xr.DataArray(maskmod0, dims=("y", "x"))   
        mask_stacked = xr_maskmod.stack(profile=('x', 'y'))
        xr_depthmod2d=xr.DataArray(depthmod2d, dims=('y','x'))
        depth_stacked = xr_depthmod2d.stack(profile=('x', 'y'))
        ...
        xr_tempmod=xr.DataArray(tempmod, dims=("time_counter","deptht","y", "x"))   
        model_temperature_stacked = xr_tempmod.stack(profile=('x', 'y'))
        xr_saltmod=xr.DataArray(saltmod, dims=("time_counter","deptht","y", "x"))   
        model_salinity_stacked = xr_saltmod.stack(profile=('x', 'y'))
        ...
        temp_model_mean_depobs=np.interp(obsred_dep,np.array(depthmod),temp_model_mean)
        temp_model_percentile_10_depobs=np.interp(obsred_dep,np.array(depthmod),temp_percentile_10)
        temp_model_percentile_90_depobs=np.interp(obsred_dep,np.array(depthmod),temp_percentile_90)
        salt_model_mean_depobs=np.interp(obsred_dep,np.array(depthmod),salt_model_mean)
        salt_model_percentile_10_depobs=np.interp(obsred_dep,np.array(depthmod),salt_percentile_10)
        salt_model_percentile_90_depobs=np.interp(obsred_dep,np.array(depthmod),salt_percentile_90)
